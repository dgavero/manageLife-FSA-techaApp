...
class Test_PersonalInformationPages(BaseTest):
    ...
    @pytest.mark.parametrize("middlename,zipcode", dataProvider.get_data("personal_info"))
    def test_updateInfo(self, middlename, zipcode, username, password):

        login = LoginScreen(self.driver)
        personal_info = PersonalInformationPage(self.driver)

        login.techLogin(username, password)
        personal_info.editMiddleName(middlename, zipcode)
